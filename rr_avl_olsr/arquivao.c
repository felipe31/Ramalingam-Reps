...
typedef struct edge {
    int head_vertex, cost, tail_vertex;
    char hot_line;
    struct edge *next_pred;
    struct edge *next_adj;
} edge;
...
heap * heap_new()
{
 ...
 if(!_routing_tree) return 
# 63 "main.c" 3 4
                          ((void *)0)
# 63 "main.c"
                              ;

 avl_init(_routing_tree, cmp_key, 
# 65 "main.c" 3 4
                                 0
# 65 "main.c"
                                      );
...
int heap_insert(node * node_to_insert, heap * queue)
{
 if(!queue || !node_to_insert) return -1;

 struct avl_node * avl_n = calloc (1, sizeof(struct avl_node));
 if(!avl_n) return -1;

 avl_n->key = node_to_insert;
...
int main(void)
{
    ...
    heap_insert(n, queue);
    heap_insert(n2, queue);

    void * a = ({ const __typeof__(((__typeof__(*(node)) *)0)->avl_node.list) *__tempptr = ((queue)->list_head.next); (__typeof__(*(node)) *)((uint8_t *)__tempptr - 
# 99 "main.c" 3 4
              __builtin_offsetof (
# 99 "main.c"
              __typeof__(*(node))
